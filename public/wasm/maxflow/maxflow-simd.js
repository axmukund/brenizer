(function registerBrenizerMaxflowSimdLoader() {
      ...
      async solve(args) {
        ...
        try {
          ...
          if (rc !== 0) {
            ...
            throw new Error(`WASM maxflow solve failed (${rc}): ${message}`);
          }
  ...
  self.__brenizerMaxflowAssetLoaders.simd = async function loadBrenizerMaxflowSimd(options) {
    ...
    let factory = resolveFactory('createBrenizerMaxflowSimdModule');
    if (!factory) {
      importScripts(moduleUrl);
    }
    factory = resolveFactory('createBrenizerMaxflowSimdModule');
    if (!factory) {
      throw new Error('SIMD module factory did not register');
    }

    const Module = await factory({
      locateFile(path) {
        if (/\.wasm($|\?)/.test(path)) return options.wasmUrl;
        if (/\.worker\.js($|\?)/.test(path)) return deriveWorkerUrl(options.scriptUrl);
        return new URL(path, moduleUrl).toString();
      },
      mainScriptUrlOrBlob: moduleUrl,
    });

    return createSolverBridge(Module, 'wasm-simd', 'compiled SIMD maxflow');
  };
})();
